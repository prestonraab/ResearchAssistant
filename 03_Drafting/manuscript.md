...
> [!question]- Table 2, Adjusters (status:: undefined)
...
> Different batch correction methods preserve or remove different aspects of the data structure, which can interact with how different classifiers learn decision boundaries. For example, some adjusters preserve non-linear relationships better than others, potentially favoring classifiers that can exploit such patterns.
...
> ![Figure 2: MCC Rank by Adjuster](../figures/mcc_rank_by_adjuster.png) *Figure 2. Horizontal violin plots illustrate the distribution of performance rankings for various batch adjustment methods across multiple cross-study validation scenarios. Methods are ordered by their pseudomedian performance, shown as grey vertical bars, with higher-performing adjusters positioned at the top. The x-axis represents the relative rank compared to other methods, where a rank of 1 (leftmost) indicates optimal performance. Gray violins represent the density of ranks across all scenarios. Colored points denote the pseudomedian rank achieved on specific classifiers (e.g., Random Forest, XGBoost) when using the corresponding adjustment method.  Outlier classifiers that deviate significantly from the method's central tendency are labeled for clarity.*

> [!question]- What does Figure 2 reveal about the change in performance? (status:: undefined)
> Figure 2 displays results for each adjuster and classifier, aggregated over test and training sets. Most adjusters have consistent performance. The top five adjusters (Combat, Rank Twice, Naive, Combat Mean, and NPN) interacted little with the top three classifiers (ElasticNet, XGBoost, and Neural Net). The top three adjusters in particular (Combat, Rank Twice, and Naive) hardly interacted with any classifier.

> [!question]- ComBat-supervised and KNN (status:: undefined)
> Other adjusters were less consistent. Supervised ComBat adjustment showed a particularly severe interaction with KNN. See also Figure 1. This suggests that KNN's distance-based learning mechanism is particularly sensitive to the specific transformations introduced by supervised batch correction. Supervised adjustment uses class labels during batch correction, preserving correlations with the labels that are common across the training sets. Noisy correlations could potentially lead to artificial separation in feature space. This could disrupt the local neighborhood by inflating the distance between true cross-batch neighbors and result in poor classification performance.

> [!question]- Supervised ComBat on LDA (status:: undefined)
> We contrast this to supervised ComBat's performance with the LDA classifier. Linear Discriminant Analysis assumes that each class is Gaussian distributed, and that the features of both clases have the same correlations. This particular implementation uses shinkage, which assumes that the features are slightly more independent than might be suggested by the data. As suggested by Figure 1, supervised ComBat is a particularly favorable adjuster for LDA. Supervised ComBat assumes Gaussian distributed classes, and actively works to preserve label correlations common to the training batches. This creates a powerful synergy: while supervised ComBat stabilizes the class-conditional distributions to be more consistent across batches, the shrinkage component of the LDA classifier compensates for any residual noise in the estimated covariance. This prevents the LDA model from over-fitting to the specific correlations reinforced by the supervised adjustment. Consequently, the classifier is able to leverage the 'cleaned' class signals provided by ComBat without becoming sensitive to the potential inflation of spurious feature dependencies.

> [!question]- NPN on LDA (status:: DRAFT)
> NPN, on the other hand, is a particularly poor choice of transformation to pair with LDA. The results of supervised Combat suggest that each class is likely normally-distributed; with two classes, this might result in a bimodal distribution in feature space. NPN's nonlinear transformation forces each feature to a unimodal standard normal distribution, disrupting the shape of the two underlying components and invalidating the assumptions of LDA.

> [!question]- MNN on KNN (status:: DRAFT)
> Finally, MNN and FastMNN both perform unnaturally well on the KNN classifier. This can be easily explained, as the neighborhood adjustments that the MNN methods make are easily recognized by the KNN classifier, but not by the other classifiers.

> [!question]- Summation (status:: DRAFT)
> The observed interactions show that classification performance is maximized when the biases of the classifier match the transformation logic of the batch correction method. However, high-performing adjusters frequently exhibit negligible interaction effects with top-tier classifiers, suggesting that performance gains are often dominated by the individual efficacy of the components rather than synergistic pairings. This should reassure pipeline assemblers that prioritizing the individual quality of adjusters and classifiers is a robust heuristic, as the inherent strengths of top-tier algorithms often mitigate the necessity for complex, case-specific pairing.
...
> [!question]- Within Study  Cross Validation  (status:: undefined)
> Figure 1 reveals interesting patterns with within-study cross validation
