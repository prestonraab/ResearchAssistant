...
**How much data does precision medicine require?** <!-- [undefined] --> Precision medicine relies on accurate predictions for patients across the spectrum of human diversity. This diversity is best captured and accounted for using large datasets aquired at great cost.

**How can dataset size be increased?** <!-- [undefined] --> Adding data from independent cohorts has the potential to improve dataset diversity, statistical power, and information via the increase in sample size. These benefits transfers to learners with increased ability to detect patterns and generalize to new data.

**What is the central challenge to combining datasets?** <!-- [undefined] --> This potential is frequently undermined by "batch effects"—systematic technical variations that can lead a learner to distinguish between experimental batches rather than meaningful biological conditions.  Batch effects can substantially degrade predictor performance when applied to new individuals.
...
**What type of data will this chapter focus on?** <!-- [undefined] --> For our purposes, and without loss of generality, we will focus on RNA sequencing (RNA-seq) data, which has become the dominant technology for measuring gene expression. <!-- Source: C_31 -->

**Why gene expression?** <!-- [undefined] --> Gene expression data provide an excellent context for understanding how batch effects can be modeled and removed, as the technical variation introduced by sequencing technologies is well-characterized and substantial. Insights from gene expression are extensible to other modalities. <!-- Source: C_07 -->
...
**How can researchers access gene expression data?** <!-- [undefined] --> The Gene Expression Omnibus (GEO) serves as a critical international public repository for such data, with over 200,000 studies and 6.5 million samples. GEO enables broad access to vast collections of gene expression and epigenomics data  in addition to relevant clinical variables. <!-- Source: C_31 -->

**What are microarrays?** <!-- [undefined] --> GEO stores data collected using both RNA-seq microarrays, an older technology which ...

**How is public gene expression data used?** <!-- [undefined] --> This wealth of data is widely reused for diverse applications, including identifying novel gene expression patterns, finding disease predictors, and developing computational methods.  <!-- Source: C_35 -->

**Why integrate gene expression datasets?** <!-- [undefined] --> Integrating gene expression data across multiple studies offers substantial benefits: increased sample sizes improve the robustness of classifiers, independent validation cohorts provide evidence of generalizability, and meta-analyses can reveal consistent patterns across diverse populations.

**How sensitive is gene expression to batch effects?** <!-- [undefined] --> However, batch effects pose particular challenges for gene expression data due to the sensitivity of sequencing technologies to technical variation. These technical artifacts can be substantial—often comparable to or larger than the biological signals of interest—making them a primary concern when combining datasets. <!-- Source: C_07 -->

**What specific effects do batches have on gene expression?** <!-- [undefined] --> Batch effects manifest in gene expression data as systematic shifts in expression levels between batches, differences in variance structure, and alterations in the relationships between genes. <!-- Source: C_07 -->
...
**What is confounding?** <!-- [undefined] --> These batch effects mimic real differences in expression levels and covariance structures between batches. Removing all differences can erase true differences between datasets. For example, one study might have a higher proportion of females than another study. It might be difficult to determine whether the differences in gene expression are technical artifact that should be removed, or true differences due to the sex imbalance between datasets. We might say that the batch effect is "confounded" with sex—the effect of the batch is somewhat tied to the effect of the sex imbalance on gene expression.

**How can confounding be resolved?** <!-- [undefined] --> Confounding can often be removed if the batches are known and the values of the confounding variables are known.

**How specifically does Combat account for confounding?** <!-- [undefined] --> 

**When is confounding difficult to overcome?** <!-- [undefined] --> If these other variables are not known, population differences can masquerade as technical artifacts.

**How should unknown batch effects be positioned?** <!-- [undefined] --> Briefly acknowledge unknown batch effects and SVA, but clearly scope them as beyond the chapter's focus on known batch correction.

**First mention of SVA: Define the problem** <!-- [undefined] --> In some cases, batch labels may be unknown or only partially known, particularly when working with publicly available data where experimental metadata may be incomplete.  Sometimes we don't know the batch labels—especially with public data where complete experimental metadata may not be provided.  Methods such as surrogate variable analysis (SVA) can identify and adjust for unknown batch effects by extracting surrogate variables from high-dimensional data that capture unwanted effects.  SVA was introduced by Leek and Storey (2007) to model unknown, latent sources of variation in genomics data, and is available in the Bioconductor sva package.  This topic extends beyond the scope of the present chapter, which focuses on supervised batch correction methods where batch labels are known.  A second discussion of SVA appears in "The Horizon" section, where we examine how SVA is being integrated into modern automated pipelines for advanced applications.

**Transition to classifiers** <!-- [undefined] --> We now transition to examining the classifiers themselves, followed by the adjustment methods designed to address batch effects, and finally the interaction between these two components in determining overall classification performance.
