...
This file contains **Method** claims related to classifier details and data-specific methods (C_75 through C_97).
...
**Primary Quote** (Section 3.1, Line 137):
...
**Supporting Quotes**:
- (Methods, Díaz-Uriarte and Alvarez de Andrés 2006): "SVM (with linear kernel, as used here) try to find an optimal separating hyperplane between the classes. When the classes are linearly separable, the hyperplane is located so that it has maximal margin (i.e., so that there is maximal distance between the hyperplane and the nearest point of any of the classes) which should lead to better performance on data not yet seen by the SVM."
- (Background, Díaz-Uriarte and Alvarez de Andrés 2006): "Random forest has excellent performance in classification tasks, comparable to support vector machines."
...
- (Abstract): "The elastic net is particularly useful when the number of predictors (p) is much bigger than the number of observations (n). By contrast, the lasso is not a very satisfactory variable selection method in the p >> n case."
...
**Primary Quote** (Abstract, Line 39):
...
**Primary Quote** (Abstract, Line 23):
...
**Primary Quote** (Discussion, Line 259):
...
**Supporting Quotes**:
- (Results): "The range of Beta-values is between 0 and 1, which can be interpreted as the approximation of the percentage of methylation for the population of a given CpG site in the sample."
...
**Supporting Quotes**:
- (Results): "The M-value is calculated as the log2 ratio of the intensities of methylated probe versus unmethylated probe"
- (Conclusions): "the M-value method is more statistically valid in differential and other statistic analysis as it is approximately homoscedastic"
- (Discussion, Line 259): "The Beta-value range is from 0 and 1 and can be interpreted as an approximation of the percentage of methlyation. However, because the Beta-value has a bounded range, this statistic violates the Gaussian distribution assumption used by many statistical methods, including the very prevalent t-test."
...
**Supporting Quotes**:
- (Abstract): "Many normalization methods commonly used in proteomics have been adapted from the DNAmicroarray techniques."
- (Abstract): "We found that variance stabilization normalization (Vsn) reduced variation the most between technical replicates in all examined data sets. Vsn also performed consistently well in the differential expression analysis. Linear regression normalization and local regression normalization performed also systematically well."
