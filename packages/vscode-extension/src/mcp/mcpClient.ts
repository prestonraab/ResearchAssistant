...
export class MCPClientManager {
  ...
  async verifyQuote(quote: string, authorYear: string): Promise<VerificationResult> {
    ...
    if (cached) {
      console.log('Returning cached verification result:', cached);
      return cached;
    }
    
    try {
      console.log('Verifying quote directly:', { quote: quote.substring(0, 100), authorYear });
      
      const result = await this.withRetry(async () => {
        // Import the quote verifier from our own extension
        const { verifyQuote: verifyQuoteFn } = await import('../core/quoteVerifier');
        ...
        const toolResult = verifyQuoteFn(quote, authorYear, extractedTextDir);
        
        console.log('Verification result:', toolResult);
        
        if (!toolResult || typeof toolResult !== 'object') {
          console.error('Invalid verification result:', toolResult);
          throw new Error('Invalid verification result');
        }
        
        const mappedResult = {
          verified: toolResult.verified || false,
          similarity: toolResult.similarity || 0,
          closestMatch: toolResult.matchedText || toolResult.nearestMatch,
          context: toolResult.contextBefore
        } as VerificationResult;
        
        console.log('Mapped verification result:', mappedResult);
