import { EmbeddingService, ClaimsManager } from '../index.js';
import {
  ClaimMatch,
  DraftAnalysis,
  SentenceAnalysis,
 ...
 * - Draft text analysis with sentence-level matching
 * - Generalization keyword detection
 * - Multi-source support finding
 * 
...
export class SearchService {
   ...
   * @returns Array of matching claims sorted by relevance
   */
   ...
   * 
   * Supports two modes:
   * - 'sentence': Analyze each sentence independently
   * - 'paragraph': Treat entire text as one unit
   * 
   * @param draftText The draft text to analyze
   * @param mode Analysis mode ('sentence' or 'paragraph')
   * @param threshold Minimum similarity threshold (0-1)
   * @returns Analysis with per-sentence results and recommendations
   */
  async searchByDraft(
    draftText: string,
   ...
   * 
   * Generalization keywords indicate claims that may need multiple sources
   ...
   * 
   * Groups matching claims by source and returns the best match from each source.
   * Useful for verifying that claims are supported by multiple independent sources.
   * 
   * @param statement The statement to verify
   * @param minSources Minimum number of sources required
   * @returns Result with supporting claims grouped by source
   */
