...
function activate(context) {
    ...
    const goToClaimCommand = vscode.commands.registerCommand('citationHover.goToClaim', async (claimData) => {
    ...
    const openExtractedTextCommand = vscode.commands.registerCommand('citationHover.openExtractedText', async (pdfBaseName, folder) => {
...
function matchesAuthorYear(filename, authorYear) {
...
function similarityScore(quote, window) {
...
async function searchZoteroForSelection() {
...
async function executeZoteroSearch(query, limit) {
...
async function displaySearchResults(query, results) {
    // Create interactive webview panel instead of static markdown
    const panel = vscode.window.createWebviewPanel('zoteroSearchResults', 'Zotero Search Results', vscode.ViewColumn.Beside, {
        enableScripts: true,
...
function getSearchResultsHtml(query, results) {
...
async function addResultToClaims(item) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
...
function getNextClaimId(existingIds) {
...
async function copyBibTeX(itemKey) {
    try {
...
async function viewFullText(itemKey) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        vscode.window.showErrorMessage('No workspace folder open');
        return;
...
async function openInZotero(itemKey) {
    if (!itemKey) {
...
async function goToClaim(claimData) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        return;
    }
...
async function showQuoteDiff(claimData) {
    // Create a webview panel showing the diff
    const panel = vscode.window.createWebviewPanel('quoteDiff', `Quote Verification: ${claimData.id}`, vscode.ViewColumn.Beside, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
...
function getQuoteDiffHtml(claimData) {
...
function generateDiffHtml(yourWords, sourceWords) {
...
async function replaceQuoteInClaims(claimData, newQuote) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        return;
    }
...
async function jumpToClaim(claimId) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        return;
    }
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            if (fs.existsSync(extractedDir)) {
                ...
                if (files.length > 0) {
                    const openFile = await vscode.window.showInformationMessage('Open extracted text?', 'Yes', 'No');
                    if (openFile === 'Yes') {
...
async function openExtractedText(pdfBaseName, folder) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        return;
    }
    ...
    const matchingFile = files.find(f => f.includes(pdfBaseName) ||
        f.includes(folder) ||
